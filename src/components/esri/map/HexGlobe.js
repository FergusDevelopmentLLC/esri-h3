...
class GeoJsonExampleAsync extends Component {
  ...
  startup = (mapConfig, node, isScene = false) => {
    console.debug('mapConfig', mapConfig)
  ...
  setupWidgetsAndLayers = async () => {
    ...
    const urShowPentagons = new UniqueValueRenderer(
      {
        field: 'class',
        uniqueValueInfos: [
    ...
    const urNoPentagons = new UniqueValueRenderer(
      {
        field: 'class',
        uniqueValueInfos: [
          {value: "pentagon", symbol: class00},
          {value: "0", symbol: class00},
          {value: "1", symbol: class01},
          {value: "2", symbol: class02},
          {value: "3", symbol: class03},
    ...
    watchUtils.whenTrue(this.view, "stationary", async () => {
      ...
      const url = `${urlPrefix}h3NoData/${box.top_left}/${box.bottom_left}/${box.bottom_right}/${box.top_right}/${this.view.zoom}`;
      
      console.log(url);

      const geoJsonLayer = new GeoJSONLayer({
        url: url,
        renderer: this.props.showPentagons ? urShowPentagons : urNoPentagons,
        outFields: ["*"]
      });
